package org.kamsystem.common.constants;

public class DtoConstants {

    public static final String GENERIC_ERROR_MSG =
        "Server issue! We are facing some difficulties, please try after some time.";

    public static final String RESOURCE_NOT_FOUND = "Something went wrong! requested resource not found";

    public static final String ERROR_MESSAGE_KEY = "ERROR_MESSAGE";

    public static final String METHOD_NOT_ALLOWED = "Method not allowed";

    public static final String JSON_PARSING_ERROR = "Error while parsing JSON";

    public static final String INVALID_REQUEST_ARGUMENTS = "Invalid request";
}
